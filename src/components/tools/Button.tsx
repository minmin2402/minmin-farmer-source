...
export const ExcelImportVideoMKT: React.FC<ExcelImportProps> = ({ onImportSuccess }) => {
...
export const ExcelImportAutoPost: React.FC<ExcelImport2Props> = ({ onImportSuccess }) => {
  // 🎯 Dùng Ref để điều khiển ô Input ẩn
  const fileInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleFileUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Map data về chuẩn Task của ông
        // 2. Map dữ liệu (Bỏ qua dòng 0 nếu đó là dòng tiêu đề, dùng .slice(1))
        const formattedTasks: AutoShopeeTask[] = data.slice(1).map((row, index) => {        
