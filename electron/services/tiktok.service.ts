...
export class TiktokService {
    static async getInfoProduct(_event: Electron.IpcMainInvokeEvent, gpmClient: GpmService, port: number, delay_between: any, data: any, task: any): Promise<any> {
        ...
        try {
            ...
            newProfile = await gpmClient.createProfile("tiktok")
    ...
    static async puppeteerCollect(port: number, data: any) {
        try {
            ...
            const xpathTitle = '//span[@class="H2-Semibold text-color-UIText1Display"]';
            ...
            if (data.task.productUrl.includes("vt.tiktok.com")) {
                try {
                    // 1. Đi đến link vt.tiktok.com và đợi nó chuyển hướng (Redirect) xong
                    await page.goto(data.task.productUrl, {
                        waitUntil: data.configVideoMKT?.method_load_page ?? "load",
                        timeout: 30000
                    });

                    // 2. Lấy URL hiện tại sau khi trình duyệt đã bung link
                    const currentUrl = page.url();

                    // Kiểm tra xem nó có bung đúng ra dạng link sản phẩm không
                    if (currentUrl.includes("tiktok.com/view/product/")) {
                        // 3. TUYỆT KỸ LỘT ĐỒ: Dùng object URL để vứt bỏ hoàn toàn phần đuôi tracking (?)
                ...
                try {
                    ...
                    logger.error(`⚠️ Lỗi khi truy cập link trực tiếp: ${error}`);
                }
            }
            ...
            try {
                ...
                await page.waitForSelector(`xpath/${xpathTitle}`, {
                    timeout: data.configVideoMKT?.time_wait_getdata ?? 30000
                });
