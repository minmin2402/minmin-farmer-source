...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  runTaskAutoImage: (data: any) => ipcRenderer.invoke('image:run-tasks', data),

  stopTaskAutoVideo: (taskIds: any) => ipcRenderer.send('video:stop-single-task', taskIds),
  stopTaskAutoImage: () => ipcRenderer.invoke('image:stop-task'),
  ...
  onTaskImageLog: (callback: (data: any) => void) => {
    const subscription = (_event: any, data: any) => callback(data);

    // Đăng ký lắng nghe sự kiện 'video:task-log'
    ipcRenderer.on('image:task-log', subscription);

    // Trả về một hàm để "hủy" lắng nghe (Cleanup) tránh rò rỉ bộ nhớ
    return () => {
      ipcRenderer.removeListener('image:task-log', subscription);
    };
  },
  ...
  pinVideo: (videoPath: string[],folderStore:string) => ipcRenderer.invoke('library:pin', videoPath,folderStore),
  deleteVideo: (id: any, absolutePath: any) => ipcRenderer.invoke('library:delete', id, absolutePath)
});
