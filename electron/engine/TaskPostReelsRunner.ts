...
export class TaskPostReelsRunner {
    ...
    private async processSingleTask(task: PostReelsTask, api_gpm: string, port: number, windowSize: string, // Mặc định size
        ...
        try {
            ...
            await this.smartType(page, task.description)
            ...
            try {
                ...
                if (commentBoxes.length > 0) {
                    ...
                    await this.smartType(page, `Mua ở đây nè ${task.affiliate}`)
    ...
    private async smartType(page: any, text: string) {
        for (const char of text) {
            await page.keyboard.type(char);
            // Ngẫu nhiên từ 30ms đến 150ms mỗi phím
            const delay = Math.floor(Math.random() * 120) + 30;
            await new Promise(r => setTimeout(r, delay));
        }
    }
    private checkStop(taskId: string) {
        if (this.stoppedTaskIds.has(taskId)) {
            throw new Error("STOP_REQUESTED"); // Quăng lỗi để nhảy vào finally đóng trình duyệt ngay
        }
    }
