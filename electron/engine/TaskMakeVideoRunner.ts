...
export class TaskRunner {
    ...
    async execute() {
        ...
        const { engine, profiles_veo3, prompt_review,isUseCharacter, characterSelected, voice_code, vbee_app_token, useProfileAff, isUseVbee, isEnabledLogo, isEnabledMusic, musicPath, logoPath, vbee_app_id, save_shopid_productid, profiles_aff, thread, apikey_gemini, delay_between, profiles_grok, prompt_image, output_video, prompt_video, speed_voice, isUseVoice } = configVideoMKT;
        ...
        const promises = tasks.map((task: any, index: number) => {
            return limit(async () => {
                ...
                try {
                    ...
                    if (task.mode === "Chỉ lấy thông tin sản phẩm" || task.mode.includes("TT")) {
                        await deletePath(task.productPathImg)
                    }


                    // Checkpoint: Nếu chỉ lấy thông tin thì dừng tại đây
                    if (task.mode === "Chỉ lấy thông tin sản phẩm") {
                        this.event.sender.send('video:task-log', { status: 'success', message: 'Lấy data sản phẩm thành công!', taskId: task.id });
                        return null;
                    }

                    let resultGenPrompt: any[] = []
                    // --- CHẶNG 2: TẠO PROMPT AI (GEMINI) ---
                    resultGenPrompt = await retryStep(async () => {
                        if (this.stoppedTaskIds.has(task.id)) throw new Error("CANCELLED");
                    ...
                    const imageAIPath = await retryStep(async () => {
                        ...
                        this.event.sender.send('video:task-log', {
                            status: 'processing',
                            message: `Chuẩn bị tạo ảnh ${engine}`,
                            taskId: task.id,
                        ...
                        let genderPrompt=""
                        if (isUseVoice){
                            genderPrompt = voice_code.includes("female") ? "realistic female host" : "realistic male host";

                        }
                        const finalImagePrompt = prompt_image + `\n ${genderPrompt}`;

                        if (engine === 'veo3') {
                            // 🚀 LUỒNG VEO 3.1 VẼ ẢNH
                            /* await veoService.initHeaderVeo(
                                this.event,
                                task.id,
                                gpmClient,
                                profiles_veo3[0], // Dùng profile đầu tiên trong list Veo3 để lấy token
                                5000 + (index % 100),
                                delay_between
                            ); */
                            ...
                            const initRes = await grokService.initHeaderGrok(sendLog, task.id, grokService, grokManager, gpmClient, profiles_grok[profileNum], port, delay_between);
                    ...
                    if (!imageAIPath || !fs.existsSync(imageAIPath)) {
                        ...
                        if (task.mode.includes('Ảnh AI')) {
                            await deletePath(task.aiImagePath)
                        }
                    ...
                    await deletePath(task.finalVideoPath)
