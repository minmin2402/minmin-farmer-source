...
import { app, BrowserWindow, dialog, globalShortcut, ipcMain, IpcMainInvokeEvent, protocol, shell } from 'electron'
...
export function setupTaskAffHandle() {
  ...
  ipcMain.handle('image:run-tasks', async (_event: IpcMainInvokeEvent, data: any) => {
    if ((await new LicenseService().checkKey()).status) {
      const runner = new TaskRunnerImage(_event, data);
      return await runner.execute();
    }
...
app.whenReady().then(() => {
  ...
  createWindow();
  protocol.registerFileProtocol('box-media', (request, callback) => {
    const url = request.url.replace('box-media://', '');
    try {
  ...
  // 2. KHỞI TẠO VÀ ĐĂNG KÝ CONTROLLER KHO VIDEO
  const videoLibraryController = new VideoLibraryController();
  videoLibraryController.registerHandlers();
